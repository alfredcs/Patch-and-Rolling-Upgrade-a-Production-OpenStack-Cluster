...
default_listen_port="8083"
default_auth_protocol="http"
controller_conf_file="/etc/contrail/contrail-controller.conf"
...
def usage(message=None):
  ...
  print "-f|--file: dir and filename of the Contrail Controller config file"
  sys.exit(-1)
...
(opts, args) = getopt.getopt(sys.argv[1:], "f:hv", ["Collector API config file", "help", "verbose"]) 

for o, a in opts:
  if o in ["-h", "--help"]:
	usage()
  elif o in ["-f", "--file"]:
	controller_conf_file=a
  elif o in ["-v", "--verbose"]:
        verbose=1

#import pdb;pdb.set_trace()
cfg_parser_contrail = ConfigParser.ConfigParser()
#clen_contrail = len(cfg_parser_contrail.read(controller_conf_file))
...
params=urlencode({})
conn = httplib.HTTPConnection(listen_ip_addr, port=listen_port, timeout=50)
...
try:
    ...
    if not response:
        raise Exception("Query Controller service failed. No response")
    elif re.search('unavailable', response, re.IGNORECASE):
	raise Exception("Query Controller service failed. Service unavailable")
    else:
	...
	print
	print "Controller Service checked!"
    	sys.exit(STATE_OK)
except Exception as e:
    print "HTTP/1.1 503 Service Unavailable"
    print "Content-Type: Content-Type: text/plain"
    print
    print "<html><body><h1>503 Controller Service Unavailable</h1></body></html>"
    print str(e)
