...
params=urlencode({})
conn = httplib.HTTPConnection("127.0.0.1", port=bind_port, timeout=50)
...
try:
	...
	if ( verbose > 0 ):
		...
		for kk in range(len(jj['networks'])):
			print("     Admin State:%s, Network ID: %s, Tenant ID:%s, Status: %s" %
				(jj['networks'][kk]['admin_state_up'],  jj['networks'][kk]['id'],  jj['networks'][kk]['tenant_id'],  jj['networks'][kk]['status']))
	print
