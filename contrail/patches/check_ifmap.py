#!/usr/bin/env python
import sys,os,json,re,socket,getopt
import ConfigParser
import httplib
...
class check_ifmap(object):
  ...
  def __init__(self, args_str=None):
	...
	try:
	  ...
	  if ifmap_count_add < 3:
                raise Exception("IFMAP mismatch found at VN addition! Should be >3 entries but shown %d." % ifmap_count_add)
	  self._delete_dummy_network(auth_token_id[1], auth_token_id[0], neutron_respond, local_hostname, contrail_listen_port)
          if self._check_cassandra("default-domain:"+admin_tenant_name+":"+del_network_name+":"+neutron_respond, local_hostname, cassandra_listen_port) > 0:
                raise Exception("Contrail API and Cassandra mismatch found at VN delete!")
	  ...
	  if  ifmap_count_del > 3:
		raise Exception("IFMAP mismatch found after VN deletion! Should less than 4  entries but shown %d." % ifmap_count_del )
	  else:
  ...
  def _create_dummy_network(self,admin_tenant_name, auth_token, local_hostname, del_network_name, neutron_listen_port):
	...
	conn_neutron = httplib.HTTPSConnection(host=local_hostname, port=neutron_listen_port, timeout=2)
  ...
  def _check_ifmap_count(self, admin_tenant_name, auth_token, local_hostname, del_network_name, ifmap_listen_port):
	headers = {"X-Auth-Project-Id": admin_tenant_name, "Content-Type": "application/json", "X-Auth-Token": auth_token }
	conn_ifmap = httplib.HTTPConnection(host=local_hostname, port=ifmap_listen_port, timeout=2)
	params=urlencode({})
	...
	try:
          ...
          print str(e)
          sys.exit(STATE_CRITICAL)
	return conn_ifmap.getresponse().read().count(del_network_name)
  ...
  def _delete_dummy_network(self, admin_tenant_name, auth_token, dummy_network_id, local_hostname, contrail_listen_port):
	...
	conn_contrail = httplib.HTTPSConnection(host=local_hostname, port=contrail_listen_port, timeout=2)
	params=urlencode({})
