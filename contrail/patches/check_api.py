...
try:
    ...
    if not response:
        raise Exception("Query Contrail service failed. No response")
    elif re.search('unavailable', response, re.IGNORECASE):
	raise Exception("Query Contrail service failed. Service unavailable")
    else:
	...
	print
	print "Contrail API Service checked!"
    	sys.exit(STATE_OK)
except Exception as e:
    print "HTTP/1.1 503 Service Unavailable"
    print "Content-Type: Content-Type: text/plain"
    print
    print "<html><body><h1>503 Contrail Service Unavailable</h1></body></html>"
    print str(e)
    print "Contrail API failed!"
    sys.exit(STATE_CRITICAL)
