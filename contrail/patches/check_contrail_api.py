...
try:
    ...
    params=urlencode({})
    conn = httplib.HTTPSConnection("127.0.0.1", port=listen_port, timeout=50)
	...
	if ( verbose > 0 ):
		...
		for kk in range(len(jj['virtual-networks'])):
			print("     Virtual Network Name:%s, API URL: %s" %
                                (jj['virtual-networks'][kk]['fq_name'],  jj['virtual-networks'][kk]['href']))
	print
