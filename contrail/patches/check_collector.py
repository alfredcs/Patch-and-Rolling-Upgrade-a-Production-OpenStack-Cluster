...
for o, a in opts:
  if o in ["-h", "--help"]:
	...
	collector_conf_file=a
  elif o in ["-v", "--verbose"]:
        verbose=1

#import pdb;pdb.set_trace()
cfg_parser_contrail = ConfigParser.ConfigParser()
#clen_contrail = len(cfg_parser_contrail.read(collector_conf_file))
...
try:
    ...
    if not response:
        raise Exception("Query Collector service failed. No response")
    elif re.search('unavailable', response, re.IGNORECASE):
	raise Exception("Query Collector service failed. Service unavailable")
    else:
	...
	print
	print "Collector Service checked!"
    	sys.exit(STATE_OK)
except Exception as e:
    print "HTTP/1.1 503 Service Unavailable"
    print "Content-Type: Content-Type: text/plain"
    print
    print "<html><body><h1>503 Collector Service Unavailable</h1></body></html>"
    print str(e)
