...
if [[ -f  /etc/nova/nova.conf ]]; then
	...
	######
	if [[ -f /etc/glance/glance-api.conf ]]; then
		[[ -f /etc/haproxy/haproxy.cfg && ${local_ip_addr} == `grep horizon-api-1 /etc/haproxy/haproxy.cfg |cut -d: -f1| awk '{print $3}'|tr -d ' '` ]] &&  crontab -l | { cat; echo "3,33 * * * *  /usr/bin/keystone-manage token_flush >/dev/null 2>&1"; } | crontab -
		[[ -f /etc/haproxy/haproxy.cfg && ${local_ip_addr} == `grep horizon-api-2 /etc/haproxy/haproxy.cfg |cut -d: -f1| awk '{print $3}'|tr -d ' '` ]] &&  crontab -l | { cat; echo "13,43 * * * *  /usr/bin/keystone-manage token_flush >/dev/null 2>&1"; } | crontab -
		[[ -f /etc/haproxy/haproxy.cfg && ${local_ip_addr} == `grep horizon-api-3 /etc/haproxy/haproxy.cfg |cut -d: -f1| awk '{print $3}'|tr -d ' '` ]] &&  crontab -l | { cat; echo "23,53 * * * *  /usr/bin/keystone-manage token_flush >/dev/null 2>&1"; } | crontab -
	fi
