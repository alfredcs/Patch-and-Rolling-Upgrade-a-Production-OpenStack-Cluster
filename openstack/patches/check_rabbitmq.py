...
with open("/etc/nova/nova.conf", "r") as file_hd:
  for a_line in file_hd:
	if re.search(r'^#', a_line):
		continue
        if re.search(r'rabbit_host', a_line):
                rabbit_host=re.sub("\n", "", a_line.split('=')[1]).strip()
	if re.search(r'rabbit_port', a_line):
		rabbit_port=re.sub("\n", "", a_line.split('=')[1]).strip()
	if re.search(r'rabbit_user', a_line):
		userid=re.sub("\n", "", a_line.split('=')[1]).strip()
        if re.search(r'rabbit_password', a_line):
                password=re.sub("\n", "", a_line.split('=')[1]).strip()
        if re.search(r'rabbit_virtual_host', a_line):
                virtual_host=re.sub("\n", "", a_line.split('=')[1]).strip()
file_hd.close()

try:
	amqp_url=rabbit_host+":"+rabbit_port
